...
const { ANY, NA } = require('./Types/index');
...
/* TODO: Currently, this format holds quoted values. It's unclear exactly what to do
   with output with NA and other protected keywords.
*/
class CPE2_3_WFN {
  constructor(cpeString) {
    this.VALID_ATTRS = VALID_ATTRS;

    const attrs = CPE2_3_WFN.parseWfnString(cpeString);

    // Ensure every attribute is valid for WFN
    // TODO: Each attr should exist only once
    if (!Object.getOwnPropertyNames(attrs).every((attrName) => (
  ...
  static parseWfnString(cpeString) {
    ...
    semantics.addAttribute('attributes', {
      ...
      nonemptyListOf(part, _, nonPart) {
        const retNonPart = nonPart.attributes.reduce((acc, vals) => {
        ...
        const retPart = { part: part.attributes.part.replace(/^"(.*)"$/, '$1') };

        return { ...retPart, ...retNonPart };
      },
  ...
  static generateWfnStringFromAttributes(attrs) {
    ...
    const generateParamsString = () => VALID_ATTRS.reduce((acc, attr) => {
      /* TODO: This is technically a protocol violation as it
          makes protected values 'ANY' and 'NA', but it only
          relates to this specific function. If you need
          relates to this specific function. If you need
      */
      if (attrs[attr]) {
        switch (attrs[attr]) {
